...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['add_ebook'])) {
        ...
        $sql = "INSERT INTO ebooks (title, author, genre, year, url) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sssis', $title, $author, $genre, $year, $url);
        $stmt->execute();
        echo "eBook added successfully.";
    }
    ...
    if (isset($_POST['edit_ebook'])) {
        ...
        $sql = "UPDATE ebooks SET title=?, author=?, genre=?, year=?, url=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sssisi', $title, $author, $genre, $year, $url, $id);
        $stmt->execute();
        echo "eBook updated successfully.";
    }
    ...
    if (isset($_POST['delete_ebook'])) {
        ...
        $stmt->execute();
        echo "eBook deleted successfully.";
    }
        ...
        <h2>Manage eBooks</h2>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<form method='POST' class='edit-form' style='display:inline;'>
                        ...
                        <button type='submit' name='edit_ebook'>Edit</button>   <button type='submit' name='delete_ebook'>Delete</button>
                      </form>";
                echo "</div>";
            }
            ?>
